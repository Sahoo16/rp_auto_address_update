 ...
 */
class Address_Auto_Complete_Public {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(  'gmap_api_script',  'https://maps.googleapis.com/maps/api/js?key='.$key.'&libraries=places', array(  ), false );
	...
	public function auto_address_fillup() {
		...
		$data = get_option( 'rpress_settings', array() );
		$is_auto_address_enable = isset( $data['auto_adress_enable'] ) ? true : false;
		...
		if ( $service_type == 'delivery' && $is_auto_address_enable ) :
			...
			<p class = 'rp-col-md-6 rp-col-sm-12 rpress-checkout-fields  rpress-street-address'>
				 ...
				 <label class= 'rpress-street-address' for= 'rpress-gmap-api-field'> Selected Adress <input class= 'rpress-input pac-target-input' id= 'rpress-gmap-api-field'>
				 <label>
